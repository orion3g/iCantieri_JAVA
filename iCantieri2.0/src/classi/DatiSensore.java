	...
	// Funzione che restituisce tutti i Dati Sensore
		...
		// Funzione che restituisce tutti i Dati Sensore per cantiere
				...
				public List<DatiSensore> getDatiPerCantiere(int idCant) throws IOException, SQLException {
					...
					String query = "select sensore.idsensore, datisensore.datorumore, datisensore.datogas, datisensore.datag, datisensore.allarme from sensore join datisensore on sensore.idsensore=datisensore.idsens join area on area.idarea=sensore.idarea where  area.idcant=?"; // Query in SQL
					if (conn != null) {
						pstmt = conn.prepareStatement(query);
